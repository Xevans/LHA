...
import { PublisherFilterContext } from "../../contexts/searchFilters/publisherFilter.context";

const SearchResults = (props) => {

    const { results } = props

    const { publisherFilter, updatePublisherFilter } = useContext(PublisherFilterContext);
    ...
    // check context to determine the publisher to filter. The context will be set by the dropdown child component. 
    ...
    const options = ["All", "Grosse Pointe News", "Grosse Pointe Magazine", "Grosse Pointe Civic", "Grosse Pointe Heritage", "Grosse Pointe Review"]
    ...
    return (
        <div>
            ...
            <div className="max-w-5xl mx-auto mt-10">
                <div>
                    <Dropdown options={options} dropdown_name={getDropdownName()} />
                </div>
                <div className="grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 place-content-center gap-2">
                    ...
                    {publisherFilter !== "" &&
                        results.filter((item) => determinePublisher(item.tags) === publisherFilter).map( (obj, key) => {
                        return (
