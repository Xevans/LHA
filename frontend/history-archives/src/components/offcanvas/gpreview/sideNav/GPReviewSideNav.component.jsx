    ...
    useEffect(() => {
        ...
        try {
            async function getHighAndLow() {
                setIsLoading(true);
                try {
                    let response_high = await axios.get(`${import.meta.env.VITE_APP_EXPRESS_SERVER}/gp_review/high`);
                    const high = response_high.data.data;
                    //console.log(high[0].publishYear);
                    setUpperBound(high[0].publishYear);

                    let response_low = await axios.get(`${import.meta.env.VITE_APP_EXPRESS_SERVER}/gp_review/low`);
            ...
            if (isloading) {
                ...
                return (
                    <>
                        ...
                        <h3 className="my-3 font-bold dark:text-white"> Grosse Pointe Review</h3>
